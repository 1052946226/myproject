...
def dealwith():
    ...
    此函数为aprioria算法的主程序
    """
    data_set = loadDataSet()#载入数据
#    seeview(data_set)
    if data_set==[]:
        seeview( u"请选择数据集")
        return
    L, support_data = generate_L(data_set, k=2, min_support=0.2)#生成所有的频繁项集。
#    seeview(L)
    big_rules_list = generate_big_rules(L, support_data, min_conf=0.7)#从大的频繁项集产生的规则集
    ...
    seeview("Big Rules")
    seeview( "\n")
    for item in big_rules_list:#输出找到的强关联规则
        seeview(str(item[2])+"\t"),
